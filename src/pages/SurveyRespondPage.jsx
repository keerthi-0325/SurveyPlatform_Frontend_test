...
function QuestionField({ question, register, errors }) {
  ...
  return (
    <div className="card">
      <p className="font-medium text-gray-900 mb-3 text-sm sm:text-base leading-snug">
        {question_text}
        {is_required && <span className="text-red-500 ml-1">*</span>}
      </p>

      {question_type === 'text' && (
        <textarea className="input resize-none text-base" rows={3} placeholder="Your answer…"
        ...
        <div className="flex gap-2 sm:gap-3 flex-wrap">
          {[1,2,3,4,5].map((n) => (
            <label key={n} className="flex flex-col items-center gap-1 cursor-pointer">
              ...
              <span className="w-12 h-12 sm:w-11 sm:h-11 rounded-full border-2 border-gray-300 peer-checked:border-indigo-600 peer-checked:bg-indigo-600 peer-checked:text-white flex items-center justify-center text-sm font-semibold text-gray-600 transition-all hover:border-indigo-400 touch-manipulation">{n}</span>
            </label>
          ))}
        </div>
      )}
        ...
        <div className="flex gap-1.5 sm:gap-2 flex-wrap">
          {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
            <label key={n} className="cursor-pointer">
              <input type="radio" value={n} className="sr-only peer"
                {...register(`answers.${question_id}.value_number`, rules)} />
              <span className="w-10 h-10 sm:w-9 sm:h-9 rounded-lg border-2 border-gray-300 peer-checked:border-indigo-600 peer-checked:bg-indigo-600 peer-checked:text-white flex items-center justify-center text-sm font-semibold text-gray-600 transition-all touch-manipulation">{n}</span>
            </label>
          ))}
        </div>
      )}
      ...
      {question_type === 'yes_no' && (
        <div className="flex gap-3 sm:gap-4">
          {['Yes','No'].map((opt) => (
            <label key={opt} className="flex items-center gap-2 cursor-pointer touch-manipulation py-1">
              <input type="radio" value={opt} className="w-4 h-4" {...register(`answers.${question_id}.value_text`, rules)} />
      ...
      {question_type === 'mcq' && Options?.map((opt) => (
        <label key={opt.option_id} className="flex items-center gap-3 mb-3 cursor-pointer group touch-manipulation py-0.5">
          <input type="radio" value={opt.option_id} className="w-4 h-4 flex-shrink-0" {...register(`answers.${question_id}.selected_option_id`, rules)} />
      ...
      {question_type === 'dropdown' && (
        <select className="input text-base" {...register(`answers.${question_id}.selected_option_id`, rules)}>
      ...
      {question_type === 'date' && (
        <input type="date" className="input text-base" {...register(`answers.${question_id}.value_text`, rules)} />
      )}
...
export default function SurveyRespondPage() {
  ...
  const channel          = searchParams.get('ch') || 'link';
  ...
  if (error || !survey) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="text-center max-w-sm w-full">
        ...
        <p className="text-gray-500 mt-2 text-sm sm:text-base">This survey is not found or is no longer accepting responses.</p>
      </div>
    </div>
  );
  ...
  if (submitted) return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 to-white px-4">
      <div className="text-center max-w-sm w-full">
        ...
        {responseId && (
          <p className="text-xs text-gray-400 mt-4 font-mono break-all px-2">Response ID: {responseId}</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-white py-6 sm:py-10 px-4">
      <div className="max-w-2xl mx-auto">

        {/* Header */}
        <div className="text-center mb-6 sm:mb-8">
          <div className="inline-flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 bg-indigo-600 rounded-2xl mb-4 shadow-lg">
            <FileText size={22} className="text-white" />
          </div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 px-2 leading-tight">{survey.title}</h1>
          {survey.description && (
            <p className="text-gray-500 mt-2 text-sm sm:text-base px-2">{survey.description}</p>
          )}
          {survey.welcome_message && (
            <div className="mt-4 bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3 text-sm text-indigo-700 text-left">
              {survey.welcome_message}
            </div>
          )}
        ...
        {questions.length === 0 ? (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
            ...
            <button
              ...
              className="btn-primary w-full flex items-center justify-center gap-2 py-4 sm:py-3.5 text-base font-semibold rounded-xl touch-manipulation"
            >
